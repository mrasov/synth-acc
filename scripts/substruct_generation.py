...
class HeterocycleGenerator:
    def __init__(
        self, ring_size=5, min_subs=2, max_subs=3, max_n_skeleton=2, substituents=None
    ):
        ...
        self.substituents = (
            substituents if substituents else list(SUBSTITUENT_CATALOG.keys())
        )
    ...
    def _align_to_center(self, sequence):
        ...
        return (
            list(sequence)
            if center_idx == -1
            else sequence[center_idx:] + sequence[:center_idx]
        )
    ...
    def generate_library(self):
        ...
        for core in tqdm(unique_cores, desc="Generating L3 (Masks)"):
            free_pos = [
                i for i, atom in enumerate(core) if ATOM_CONFIG[atom]["has_sub"]
            ]
                ...
                for indices in combinations(free_pos, k):
                    ...
                    for i, atom in enumerate(core):
                        tag = (
                            "*"
                            if i in indices
                            else ("H" if ATOM_CONFIG[atom]["has_sub"] else None)
                        )
        ...
        for mask in tqdm(unique_masks, desc="Generating L4 (Substituents)"):
            ...
            def get_base_smarts(seq):
                ...
                return (
                    self._build_smarts(pure_seq, level=1),
                    ...
                    final_data.append(
                        {
                            ...
                            "layer4_smarts": self._build_smarts(
                                final_struct_resolved, level=4
                            ),
                        }
                    )
...
if __name__ == "__main__":
    generator = HeterocycleGenerator(
        ring_size=5, min_subs=2, max_subs=3, max_n_skeleton=2
    )
